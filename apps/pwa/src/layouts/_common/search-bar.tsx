...
import { NavbarFields } from "../../router";
...
export default function SearchBar() {
  ...
  const flattenedRoutes = useFlattenedRoutes(RouteObjectWithNavbar);
  ...
  const [searchResult, setSearchResult] = useState<NavbarFields[]>(
    flattenedRoutes.filter((route) => route.navLabel)
  );
  ...
  useEffect(() => {
    const result = flattenedRoutes.filter(
      (item) =>
        item.navLabel &&
        (item.navLabel.toLowerCase().indexOf(searchQuery.toLowerCase()) !==
          -1 ||
          (item.navPath || "")
            .toLowerCase()
  ...
  useKeyPressEvent("Enter", (event) => {
    ...
    event.preventDefault();
    const selectItem = searchResult[selectedItemIndex].navPath;
    if (selectItem) {
  ...
  const handleSelect = (navPath: string) => {
    replace(navPath);
    handleCancel();
  };
            ...
            <div ref={listRef} className="py-2">
              {searchResult.map((item, index) => {
                const partsTitle = parse(
                  item.navLabel || "",
                  match(item.navLabel || "", searchQuery)
                );
                const partsKey = parse(
                  item.navPath || "",
                  match(item.navPath || "", searchQuery)
                );
                return (
                  <StyledListItemButton
                    ...
                    onClick={() => handleSelect(item.navPath || "")}
